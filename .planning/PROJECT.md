...
- [ ] Clean architecture: separate color math, persistence, UI, bridge client, and calibration orchestration (no monolithic 2k-line single file)
- [ ] v1 integration: GrandMA3 plugin + Pi bridge on show LAN (HTTP, no HTTPS on console)
...
- [ ] Merge path: single canonical branch snapshot (reconcile experimental vs research Sekonic branches)
...
**Topology decision:** v1 = Pi bridge (C-7000 → USB → Pi → HTTP → plugin). Research whether direct device HTTP exists for future versions.
...
|----------|-----------|---------|
| Clean architecture, reuse behavior | Avoid another monolith + branch drift; keep proven math/flows | — Pending |
| v1 includes plugin + Pi bridge | User priority: minimal typing at FOH with C-7000 | — Pending |

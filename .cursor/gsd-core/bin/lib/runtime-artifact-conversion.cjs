...
function convertClaudeToCursorMarkdown(content) {
    ...
    // Replace "Cursor" brand references with "Cursor"
...
/**
 * Convert a Cursor command to a Cursor 1.6 slash command (#785).
 *
 ...
 *
 * @param {string} content   raw Cursor command markdown (may have frontmatter)
...
function convertClaudeToWindsurfMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Cursor bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Cursor" brand references with "Windsurf"
...
function convertClaudeToAugmentMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Cursor bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Cursor" brand references with "Augment"
 ...
 * GSD's worktree isolation uses Cursor's isolation="worktree" spawn

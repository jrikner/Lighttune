...
// .cursor/rules/ fallback / placeholder text — runtime-aware so emitted slash
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
    ...
    let targetPath;
    if (options.global) {
        targetPath = node_path_1.default.join(node_os_1.default.homedir(), '.claude', '.cursor/rules/');
    }
    else if (options.output) {
        targetPath = node_path_1.default.isAbsolute(options.output) ? options.output : node_path_1.default.join(cwd, options.output);
    }
    else {
        // Read claude_md_path from config; #1098 default is ./.claude/.cursor/rules/
        ...
        let configClaudeMdPath = './.claude/.cursor/rules/';
        try {
            const config = loadConfig(cwd);
...
function cmdGenerateClaudeMd(cwd, options, raw) {
    ...
    // `.claude/.cursor/rules/` (a valid auto-loaded memory location) rather than a
    // repo-root `.cursor/rules/`, so generated GSD content does not land next to — or
    // pollute — a hand-crafted repo-root .cursor/rules/. An explicit `claude_md_path`
    ...
    try {
        ...
        // .cursor/rules/; copilot now resolves to .github/copilot-instructions.md, and
        ...
        // rationale: a Codex/AGENTS-native project must never write to .cursor/rules/
    ...
    if (existingContent === null) {
        ...
        // section markers — it is a hand-crafted .cursor/rules//AGENTS.md, not a

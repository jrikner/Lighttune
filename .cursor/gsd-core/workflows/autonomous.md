...
- **Otherwise (Cursor or any other non-Codex runtime):** Run plan **inline** (do NOT background) so the plan-checker runs. The next phase's discuss does not overlap planning here — correctness over overlap.
...
- **Otherwise (Cursor or any other non-Codex runtime):** Run execute **inline** (do NOT background) so worktree isolation and verification run:
...
Read `human_verification` items. In text mode (`--text` or init `text_mode=true`), replace conversational prompting with a numbered list and typed choice. Otherwise display items and ask:
...
This means the user is always answering discuss questions (lightweight, interactive) while the heavy work (planning, code generation) runs in the background. The main context only accumulates discuss conversations — plan and execute contexts are isolated in their agents. (On Cursor and all other non-Codex runtimes, plan and execute run inline, so they run sequentially and their work accumulates in the main context.)
